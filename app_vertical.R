...
dirichlet_diff <- function(k, d, prop = 1, n = 1000) { 
    ...
    p <- ggplot(d, aes(x = p_diff, colour = level)) +
        stat_density(geom = "line", position = "identity") +
        facet_wrap(~ comparison) +
        labs(x = "Difference in probability", y = "Density", title = paste("k =", k)) +
    ...
    list(quantiles = mutate(q, k = k),
         ...
         post_params = mutate(bind_cols(post_params), k = k),
    ...
    fixedRow(
        column(4, 
               radioButtons("obs", "Variables", choices = list("4 levels, pairwise similar" = "levels_similar",
                                                               ...
                                                               "Custom:" = "custom")),
               textAreaInput("custom_obs", "Observed counts", value = default_obs, width = "250px", height = "15ch", label = NULL)),
        column(4, 
               textInput("k_input", "k value(s)", value = "10 50", width = "250px"),
               sliderInput("prop", "Fraction threshold", value = 100, min = 0, max = 100, step = 5, width = "250px", post = "%", ticks = FALSE),
               sliderInput("n_samples", "Number of samples", value = 1000, min = 500, max = 5000, step = 500, width = "250px", ticks = FALSE)),
        column(4, tableOutput("parsed_dat"))
    ),
    hr(),
    tabsetPanel(tabPanel("Posterior distributions", uiOutput("post_dist")),
...
server <- function(input, output) {
    ...
    output$post_dist_plot <- renderPlot({
        bind_rows(lapply(names(dir_diff()), function(.) dir_diff()[[.]]$post_samples)) %>%
            ...
            ggplot(aes(x = value, colour = level, group = level)) +
                stat_density(geom = "line", position = "identity") +
                facet_grid(k ~ cohort) +
                theme_minimal() +
                ...
                labs(x = "Proportion", y = "Density",
                     caption = "Rows: different values of k; columns: cohorts") 
    })
    
    output$post_dist <- renderUI({
        plotOutput("post_dist_plot", height = length(k()) * 200, width = "100%")
    })
    ...
    output$post_diff_plot <- renderPlot({
        bind_rows(lapply(names(dir_diff()), function(.) dir_diff()[[.]]$comparisons)) %>%
            ggplot(aes(x = p_diff, colour = factor(level))) +
                geom_vline(aes(xintercept = 0), linetype = 2, size = 0.2) +
                stat_density(geom = "line", position = "identity") +
                facet_grid(k ~ str_replace_all(comparison, "_", " ")) +
                theme_minimal() +
                theme(panel.spacing.x = grid::unit(2, "lines"),
                      panel.spacing.y = grid::unit(1, "lines")) +
                labs(x = "Difference", y = "Density", 
                     caption = "Rows: different values of k; columns: pairwise comparisons between cohorts") +
                guides(colour = guide_legend(title = "Level"))
    })
    
    output$post_diff <- renderUI({
        plotOutput("post_diff_plot", height = length(k()) * 200, width = "100%")
    })
